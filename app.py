...
def ensure_admin_user():
    ...
    Inserta si no está. (Texto plano como tu esquema actual).
    """
    conn = getConexion()
    ...
    if not row:
        try:
            ...
            conn.rollback()
            # En dev, sólo logueamos. En prod, usar logger.
...
@require_auth
def listar_usuarios():
    # Nota: sólo lectura para poblar combos (no CRUD).
    conn = getConexion()
    cur = conn.cursor(buffered=True, dictionary=True)
...
def listar_expedientes():
    ...
    if aseguradora_id is not None:
        where.append("e.aseguradora_id = %s")
    ...
    if juzgado_id is not None:
        where.append("e.juzgado_id = %s")
        params.append(juzgado_id)
    ...
    cur.execute(select_sql, tuple(params + [page_size, offset]))
    data = cur.fetchall()
...
def crear_expediente():
    datos = request.json or {}
    required = ["aseguradora_id", "usuario_id", "juzgado_id", "estado", "fecha"]
    ...
    if not fk_exists("aseguradora", datos["aseguradora_id"]):
        return json_error("aseguradora_id no existe")
    if not fk_exists("usuario", datos["usuario_id"]):
        return json_error("usuario_id no existe")
    if not fk_exists("juzgado", datos["juzgado_id"]):
        return json_error("juzgado_id no existe")

    conn = getConexion()
    cur = conn.cursor()
    try:
        cur.execute("""
            INSERT INTO expediente (aseguradora_id, usuario_id, juzgado_id, estado, fecha)
            VALUES (%s, %s, %s, %s, %s)
        """, (datos["aseguradora_id"], datos["usuario_id"], datos["juzgado_id"], datos["estado"], fecha))
        conn.commit()
...
def actualizar_expediente(e_id):
    ...
    if "aseguradora_id" in datos:
        if not fk_exists("aseguradora", datos["aseguradora_id"]):
            return json_error("aseguradora_id no existe")
        fields.append("aseguradora_id = %s")
        values.append(datos["aseguradora_id"])

    if "usuario_id" in datos:
        if not fk_exists("usuario", datos["usuario_id"]):
            return json_error("usuario_id no existe")
        fields.append("usuario_id = %s")
        values.append(datos["usuario_id"])

    if "juzgado_id" in datos:
        if not fk_exists("juzgado", datos["juzgado_id"]):
            return json_error("juzgado_id no existe")
        fields.append("juzgado_id = %s")
        values.append(datos["juzgado_id"])

    if "estado" in datos:
        if not validate_estado(datos["estado"]):
            return json_error("Estado inválido. Use: Pendiente | En Curso | Cerrado")
        fields.append("estado = %s")
        values.append(datos["estado"])
    ...
    <body>
      ...
      <script>
        ...
        async function cargarCatalogos() {
          const [ases, usrs, juzgs] = await Promise.all([
          ...
          fA.innerHTML = '<option value="">Todas</option>' + ases.map(a => `<option value="${a.id}">${a.nombre_aseguradora}</option>`).join('');
          fU.innerHTML = '<option value="">Todos</option>' + usrs.map(u => `<option value="${u.id}">${u.nombre} ${u.apellido}</option>`).join('');
          fJ.innerHTML = '<option value="">Todos</option>' + juzgs.map(j => `<option value="${j.id}">${j.nombre_juzgado}</option>`).join('');

          // Panel crear/editar (copiamos las opciones)
          document.getElementById('aseguradora_id').innerHTML = ases.map(a => `<option value="${a.id}">${a.nombre_aseguradora}</option>`).join('');
          document.getElementById('usuario_id').innerHTML = usrs.map(u => `<option value="${u.id}">${u.nombre} ${u.apellido}</option>`).join('');
          document.getElementById('juzgado_id').innerHTML = juzgs.map(j => `<option value="${j.id}">${j.nombre_juzgado}</option>`).join('');
...
if __name__ == "__main__":
    ensure_admin_user()  # crea admin/admin si no existe
